...
public sealed class AsyncMethodAnalyzer : DiagnosticAnalyzer
{
    ...
    public override void Initialize(AnalysisContext context)
    {
        ...
        context.RegisterSyntaxNodeAction(AnalyzeMethodDeclaration, SyntaxKind.MethodDeclaration, SyntaxKind.LocalFunctionStatement);
    }
    ...
    private static void AnalyzeMethodDeclaration(SyntaxNodeAnalysisContext context)
    {
        ...
        Logger.AppendLog(
            $"{nameof(AnalyzeMethodDeclaration)}: Node type: '{context.Node.GetType().Name}'. Containing symbol type: '{declaredSymbol?.GetType().Name}'.");
        ...
        static ImmutableDictionary<string, string?> CreateProperties(ISymbol symbol)
        {
            ...
            return ImmutableDictionary.CreateRange<string, string?>(
                [new KeyValuePair<string, string?>(DiagnosticPropertyNames.MethodName, methodName)]);
        }
