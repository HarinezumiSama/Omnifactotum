...
[TestFixture(TestOf = typeof(OptionalStringLengthRangeConstraintBase))]
internal sealed class OptionalStringLengthRangeConstraintBaseTests
    : TypedConstraintTestsBase<OptionalStringLengthRangeConstraintBaseTests.LengthRangeConstraint, string?>
{
    [Test]
    [TestCase(0, 0)]
    [TestCase(0, 1)]
    ...
    protected override ValidationErrorDetails GetTypedInvalidValueErrorDetails(string? invalidValue)
        => $"The string value may be null, but otherwise its length must be at least 2 and at most 5. Actual value {
            (invalidValue is null ? "is null" : $"length is {invalidValue.Length}")}.";

    internal sealed class LengthRangeConstraint() : OptionalStringLengthRangeConstraintBase(2, 5);

    private sealed class ConfigurableConstraint(int minLength, int maxLength) : OptionalStringLengthRangeConstraintBase(minLength, maxLength);
}