...
[TestFixture(TestOf = typeof(NotNullStringMinLengthConstraintBase))]
internal sealed class NotNullStringMinLengthConstraintBaseTests
    : TypedConstraintTestsBase<NotNullStringMinLengthConstraintBaseTests.MinLengthConstraint, string>
{
    [Test]
    [TestCase(0)]
    [TestCase(1)]
    ...
    protected override ValidationErrorDetails GetTypedInvalidValueErrorDetails(string? invalidValue)
        => $"The string value must not be null, and its length must be at least 3. Actual value {
            (invalidValue is null ? "is null" : $"length is {invalidValue.Length}")}.";

    internal sealed class MinLengthConstraint() : NotNullStringMinLengthConstraintBase(3);

    private sealed class ConfigurableConstraint(int length) : NotNullStringMinLengthConstraintBase(length);
}