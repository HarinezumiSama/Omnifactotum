...
[TestFixture(TestOf = typeof(OptionalStringMaxLengthConstraintBase))]
internal sealed class OptionalStringMaxLengthConstraintBaseTests
    : TypedConstraintTestsBase<OptionalStringMaxLengthConstraintBaseTests.MaxLengthConstraint, string?>
{
    [Test]
    [TestCase(0)]
    [TestCase(1)]
    ...
    protected override ValidationErrorDetails GetTypedInvalidValueErrorDetails(string? invalidValue)
        => $"The string value may be null, but otherwise its length must be at most 4. Actual value {
            (invalidValue is null ? "is null" : $"length is {invalidValue.Length}")}.";

    internal sealed class MaxLengthConstraint() : OptionalStringMaxLengthConstraintBase(4);

    private sealed class ConfigurableConstraint(int length) : OptionalStringMaxLengthConstraintBase(length);
}