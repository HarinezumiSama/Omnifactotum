...
[TestFixture(TestOf = typeof(NotNullStringFixedLengthConstraintBase))]
internal sealed class NotNullStringFixedLengthConstraintBaseTests
    : TypedConstraintTestsBase<NotNullStringFixedLengthConstraintBaseTests.FixedLengthConstraint, string>
{
    [Test]
    [TestCase(0)]
    [TestCase(1)]
    ...
    protected override ValidationErrorDetails GetTypedInvalidValueErrorDetails(string? invalidValue)
        => $"The string value must not be null, and its length must be exactly 3. Actual value {
            (invalidValue is null ? "is null" : $"length is {invalidValue.Length}")}.";

    internal sealed class FixedLengthConstraint() : NotNullStringFixedLengthConstraintBase(3);

    private sealed class ConfigurableConstraint(int length) : NotNullStringFixedLengthConstraintBase(length);
}