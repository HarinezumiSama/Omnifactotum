...
internal sealed class AsyncMethodAnalyzerTests : TestsBase
{
    public static IEnumerable<TestCaseData> AnalyzerTestCases
    {
        ...
        get
        {
            //// AsyncMethodMissingAsyncSuffix | SyncMethodHasAsyncSuffix
            ...
            yield return new TestCaseData(
                ...
                .SetName($"{DiagnosticDescriptorIds.AsyncMethodMissingAsyncSuffix}|{DiagnosticDescriptorIds.SyncMethodHasAsyncSuffix}: No diagnostics.");
            ...
            yield return new TestCaseData(
                ...
                .SetName($"{DiagnosticDescriptorIds.AsyncMethodMissingAsyncSuffix}|{DiagnosticDescriptorIds.SyncMethodHasAsyncSuffix}: Diagnostics found.");

            //// AsyncMethodMissingCancellationTokenParameter
            ...
            yield return new TestCaseData(
                    new AnalyzerTestData
                    {
                        InitialSource =
                            ...
                            public class TestClass : ITestInterface
                            {
                                public Task CorrectPublicVoidTaskAsync(string value, CancellationToken cancellationToken) => Task.CompletedTask;
                                public Task<string> CorrectPublicResultTaskAsync(string value, CancellationToken cancellationToken) => Task.FromResult(string.Empty);

                                public async ValueTask CorrectPublicVoidValueTaskAsync(string value, CancellationToken cancellationToken) => await Task.CompletedTask;
                                public async ValueTask<string> CorrectPublicResultValueTaskAsync(string value, CancellationToken cancellationToken) => await Task.FromResult(string.Empty);

                                public void CorrectPublicVoid(string value) {}
                                public string CorrectPublicResult(string value) => string.Empty;

                                internal static async IAsyncEnumerable<int> CorrectAsyncEnumerableAsync(char c, CancellationToken cancellationToken)
                                {
                                    await Task.CompletedTask;
                                    yield break;
                                }

                                private Task CorrectPrivateVoidTaskAsync(char value, CancellationToken cancellationToken) => Task.CompletedTask;
                                private Task<string> CorrectPrivateResultTaskAsync(char value, CancellationToken cancellationToken) => Task.FromResult(string.Empty);

                                private async ValueTask CorrectPrivateVoidValueTaskAsync(char value, CancellationToken cancellationToken) => await Task.CompletedTask;

                                private async ValueTask<string> CorrectPrivateResultValueTaskAsync(char value, CancellationToken cancellationToken) => await Task.FromResult(string.Empty);

                                private void CorrectPrivateVoid(char value) {}
                                private string CorrectPrivateResult(char value) => string.Empty;

                                Task ITestInterface.CorrectInterfaceVoidTaskAsync(string value, CancellationToken cancellationToken) => Task.CompletedTask;
                                public Task<string> CorrectInterfaceResultTaskAsync(string value, CancellationToken cancellationToken) => Task.FromResult(string.Empty);

                                async ValueTask ITestInterface.CorrectInterfaceVoidValueTaskAsync(string value, CancellationToken cancellationToken) => await Task.CompletedTask;
                                public async ValueTask<string> CorrectInterfaceResultValueTaskAsync(string value, CancellationToken cancellationToken) => await Task.FromResult(string.Empty);

                                public void PublicContainingMethod()
                                {
                                    Task CorrectLocalVoidTaskAsync(string value, CancellationToken cancellationToken) => Task.CompletedTask;
                                    Task<string> CorrectLocalResultTaskAsync(string value, CancellationToken cancellationToken) => Task.FromResult(string.Empty);

                                    async ValueTask CorrectLocalVoidValueTaskAsync(string value, CancellationToken cancellationToken) => await Task.CompletedTask;
                                    async ValueTask<string> CorrectLocalResultValueTaskAsync(string value, CancellationToken cancellationToken) => await Task.FromResult(string.Empty);
                                }
                            ...
                            public interface ITestInterface
                            {
                                Task CorrectInterfaceVoidTaskAsync(string value, CancellationToken cancellationToken);
                                Task<string> CorrectInterfaceResultTaskAsync(string value, CancellationToken cancellationToken);

                                ValueTask CorrectInterfaceVoidValueTaskAsync(string value, CancellationToken cancellationToken);
                                ValueTask<string> CorrectInterfaceResultValueTaskAsync(string value, CancellationToken cancellationToken);
                            }
                            """,
                        ExpectedDiagnostics = ImmutableList<DiagnosticResult>.Empty
                    })
                .SetName($"{DiagnosticDescriptorIds.AsyncMethodMissingCancellationTokenParameter}: No diagnostics.");
            ...
            yield return new TestCaseData(
                    new AnalyzerTestData
                    {
                        ...
                        ExpectedDiagnostics = ImmutableList.Create(
                            //// Methods
                            Verifier
                                .Diagnostic(DiagnosticDescriptors.AsyncMethodMissingCancellationTokenParameter)
                                .WithArguments("method", "TestClass.IncorrectPublicVoidTaskAsyn(string)")
                                .WithSpan(7, 17, 7, 44),
                            Verifier
                                .Diagnostic(DiagnosticDescriptors.AsyncMethodMissingCancellationTokenParameter)
                ...
                .SetName($"{DiagnosticDescriptorIds.AsyncMethodMissingCancellationTokenParameter}: Diagnostics found.");
        }
    }
